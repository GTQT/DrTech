...
public class ToroidBlock implements IToroidBlock {
    ...
    /** 返回所有已注册 Toroid 方块的 IBlockState（1.9.0 结构匹配用，不区分种类计数） */
    public static IBlockState[] getRegisteredStates() {
        IBlockState[] states = new IBlockState[TORUS_BLOCK_MAP.size()];
        int i = 0;
        for (ToroidBlock block : TORUS_BLOCK_MAP.values()) {
