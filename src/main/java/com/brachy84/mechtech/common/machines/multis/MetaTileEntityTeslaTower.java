...
public class MetaTileEntityTeslaTower extends MultiblockWithDisplayBase {
    ...
    /** 降级：多层重复段固定层数（原设计 3~32 可变，1.9.0 无 setRepeatable） */
    private static final int COIL_SEGMENT_REPEAT = 12;
    ...
    protected void formStructure(@NotNull FormedStructureView formed) {
        ...
        // 降级：coilHeight 按固定重复段层数计算（1.9.0 无 MatchContext 计数）
        coilHeight = COIL_SEGMENT_REPEAT;

        float amps = 0;
        ToroidBlock[] toroids = ToroidBlock.getAll().toArray(new ToroidBlock[0]);
        for (ToroidBlock block : toroids) {
            dmg += block.getDmgModifier();
            range += block.getRangeModifier();
            amps += block.getAmpsPerBlock();
        }
        // 所有已注册 Toroid 类型视为各用一组（结构 T 位 92 个方块在降级版按单 Toroid 计）
        if (toroids.length > 0) {
            dmg /= toroids.length;
            range /= toroids.length;
            amps *= TORUS_BLOCK_COUNT / Math.max(1, toroids.length);
        }
    ...
    private StructureDefinition<?> buildStructureDefinition() {
        return DeclarativePatternBuilder.start(RIGHT, FRONT, UP)
                ...
                .aisle("###########",
                        ...
                        "###########",
                        "###########")
                .aisleRepeated(COIL_SEGMENT_REPEAT,
                        "###########",
                        "###########",
                        "###########",
                        "####LLL####",
                ...
                .aisle("###########",
                        ...
                        "####TTT####",
                        "###########")
                .where('S', Elements.self(MetaTileEntityTeslaTower.class))
                .casing('C', getCasingState())
                        .energyInput(1, 8)
                        .maintenance()
                .done()
                .where('T', Elements.blocks(ToroidBlock.getRegisteredStates()))
                .tieredCasing('L', GTCasingGroups.heatingCoils().group())
                .withChannel(GTCasingGroups.heatingCoils().channel())
                .where('P', Elements.block(materialBlockState(Materials.Polyethylene)))
                .where('#', Elements.any())
                .buildStructureDefinition();
    }

    private static IBlockState materialBlockState(Material material) {
    ...
    public void renderMetaTileEntity(CCRenderState renderState, Matrix4 translation, IVertexOperation[] pipeline) {
        ...
        Textures.MULTIBLOCK_WORKABLE_OVERLAY.renderOrientedState(renderState, translation, pipeline, Cuboid6.full, getFrontFacing(), isStructureFormed() && energyHandlers.size() > 0, true);
    }
