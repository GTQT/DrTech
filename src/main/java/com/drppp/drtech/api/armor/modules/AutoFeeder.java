...
public class AutoFeeder extends AbstractModule {
    ...
    public void onClientTick(World world, EntityPlayer player, ItemStack modularArmorPiece, NBTTagCompound armorData) {
        if (toggleTimer == 0 && Keys.AUTO_FEEDER_MODE_SWITCH.isKeyDown(player) && Keys.AUTO_FEEDER_MODE_SWITCH.isPressed(player)) {
            toggleTimer = 5;
            ArmorNetworkHandler.sendToServer(new CModularArmorSwitchModuleMode(ModularArmor.get(modularArmorPiece).getEquipmentSlot(modularArmorPiece), "feeder_enabled"));
        }
        if (toggleTimer > 0) {
            toggleTimer--;
        }
