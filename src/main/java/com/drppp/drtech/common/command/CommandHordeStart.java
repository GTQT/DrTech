...
public class CommandHordeStart extends CommandBase {
    ...
    public String getUsage(@NotNull ICommandSender sender) {
        return "drtech.command.horde.start.usage";
    }
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        if(sender instanceof EntityPlayerMP) {
            ...
            if (args.length > 0) {
                ...
                if (event == null) {
                    throw new CommandException("drtech.command.horde.start.no_such_horde", name);
                }

                if (!event.canRun(player)) {
                    throw new CommandException("drtech.command.horde.start.unable_to_run", name);
                }
                ...
                if (playerData.hasActiveInvasion) {
                    ...
                    if (args.length > 1 && args[1].equals("true")) {
                        ...
                        ITextComponent textComponent = new TextComponentTranslation("drtech.command.horde.start.has_active_invasion", playerData.currentInvasion);
                ...
                if (!event.run(player, playerData::addEntity)) {
                    throw new CommandException("drtech.command.horde.start.error_executing_horde");
                }

                playerData.setCurrentInvasion(event);
                ITextComponent textComponent = new TextComponentTranslation("drtech.command.horde.start.started", event.KEY);
                sender.sendMessage(textComponent);
            } else {
                throw new CommandException("drtech.command.horde.start.argument_required");
            }
        }
    }
}