...
public final class DroneSoundLimiter {
    ...
    public synchronized boolean shouldPlay(UUID droneId,DroneSoundCue cue,long worldTime){if(droneId==null||cue==null)return false;String key=droneId+":"+cue.name();long last=lastPlayed.getOrDefault(key,Long.MIN_VALUE);if(last!=Long.MIN_VALUE&&worldTime>=last&&worldTime-last<cue.getMinIntervalTicks())return false;lastPlayed.put(key,worldTime);return true;}
