...
final class DroneWorldActions {
    ...
    static int attackEntity(EntityProgrammableDrone drone, ItemStackHandler inventory, EntityLivingBase target,
        ...
        if (!DrtConfig.drone.EnableDroneCombat || !(drone.world instanceof WorldServer) || target == null
                || !target.isEntityAlive() || !target.isNonBoss()
                || target instanceof EntityPlayer && !DrtConfig.drone.EnableDronePlayerAttack) return -1;
        FakePlayer player = preparePlayer(drone);
    ...
    static EntityFishHook castFishingHook(EntityProgrammableDrone drone, ItemStack rod, BlockPos target) {
        ...
        hook.setLuck(enchantmentLuck + Math.max(0, Math.min(100, DrtConfig.drone.DroneFishingLuckBonus)));
    ...
    private static FakePlayer preparePlayer(EntityProgrammableDrone drone) {
        ...
        DroneFakePlayerIdentity strategy = DrtConfig.drone.DroneFakePlayerIdentityStrategy == null
                ? DroneFakePlayerIdentity.PER_DRONE : DrtConfig.drone.DroneFakePlayerIdentityStrategy;
