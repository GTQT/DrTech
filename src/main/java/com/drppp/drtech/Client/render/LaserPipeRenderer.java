...
public class LaserPipeRenderer extends PipeRenderer {
    ...
    public LaserPipeRenderer() {
        super("gt_laser_pipe", new ResourceLocation("drtech", "laser_pipe"));
    }
    ...
    public void buildRenderer(PipeRenderer.PipeRenderContext renderContext, BlockPipe<?, ?, ?> blockPipe, @Nullable IPipeTile<?, ?> pipeTile, IPipeType<?> pipeType, @Nullable Material material) {
        if (pipeType instanceof LaserPipeType) {
            renderContext.addOpenFaceRender(new IVertexOperation[]{new IconTransformation((TextureAtlasSprite)this.pipeTextures.get(pipeType))}).addSideRender(false, new IVertexOperation[]{new IconTransformation(Textures.LASER_PIPE_SIDE)});
            ...
            boolean var10001;
            label21: {
                if (!ConfigHolder.client.preventAnimatedCables && pipeTile instanceof TileEntityLaserPipe) {
                    TileEntityLaserPipe laserPipe = (TileEntityLaserPipe)pipeTile;
    ...
    protected void renderOtherLayers(BlockRenderLayer layer, CCRenderState renderState, PipeRenderer.PipeRenderContext renderContext) {
        if (this.active && layer == BloomEffectUtil.getEffectiveBloomLayer() && (renderContext.getConnections() & 63) != 0) {
            Cuboid6 innerCuboid = BlockPipe.getSideBox((EnumFacing)null, renderContext.getPipeThickness());
                ...
                for(int var7 = 0; var7 < var6; ++var7) {
                    ...
                    if ((renderContext.getConnections() & 1 << side.getIndex()) == 0) {
                        ...
                        if ((renderContext.getConnections() & 1 << oppositeIndex) <= 0 || (renderContext.getConnections() & 63 & ~(1 << oppositeIndex)) != 0) {
                            ...
                            ops = (IVertexOperation[]) ArrayUtils.addAll(ops, new IVertexOperation[]{new IconTransformation(Textures.LASER_PIPE_OVERLAY_EMISSIVE)});
