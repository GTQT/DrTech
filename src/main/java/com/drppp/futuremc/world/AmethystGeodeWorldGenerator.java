package com.drppp.futuremc.world;
...
public class AmethystGeodeWorldGenerator implements IWorldGenerator {
                ...
                for (int dz = -radius; dz <= radius; dz++) {
                    ...
                    if (distance <= airRadius) {
                        ...
                        IBlockState state = random.nextDouble() < BUDDING_CHANCE
                                ? FutureMCMain.BUDDING_AMETHYST.getDefaultState()
                                : FutureMCMain.AMETHYST_BLOCK.getDefaultState();
                        world.setBlockState(pos, state, 2);
                        if (state.getBlock() == FutureMCMain.BUDDING_AMETHYST) {
                            buddingBlocks.add(pos);
                        }
                    } else if (distance <= middleRadius) {
                        world.setBlockState(pos, FutureMCMain.CALCITE.getDefaultState(), 2);
                    } else {
                        world.setBlockState(pos, FutureMCMain.SMOOTH_BASALT.getDefaultState(), 2);
                    }
                }
            }
        }
    ...
    private IBlockState getRandomBud(Random random) {
        ...
        if (roll <= 3) {
            return FutureMCMain.SMALL_AMETHYST_BUD.getDefaultState();
        }
        if (roll <= 6) {
            return FutureMCMain.MEDIUM_AMETHYST_BUD.getDefaultState();
        }
        if (roll <= 8) {
            return FutureMCMain.LARGE_AMETHYST_BUD.getDefaultState();
        }
        return FutureMCMain.AMETHYST_CLUSTER.getDefaultState();
    }
